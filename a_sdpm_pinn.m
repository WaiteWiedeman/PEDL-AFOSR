...
% Train model with matlab trainnet
opts = trainingOptions("adam",MaxEpochs=numEpochs,Plots="training-progress",...
    LearnRateSchedule="piecewise",LearnRateDropFactor=0.2,...
    LearnRateDropPeriod=10);
[netTrained,info] = trainnet(xTrain,yTrain,net,@modelLoss,opts);

fname = "pinn_modelb.mat";
save(fname,"netTrained");

%% plot training loss and RMSE
figure('Position',[500,100,800,400]); 
tiledlayout("vertical","TileSpacing","tight")
%info = monitor.MetricData.TrainingLoss;
x = info.TrainingHistory.Iteration;
y = info.TrainingHistory.Loss;
...
set(gca, 'FontSize', 15);
saveas(gcf,'training_a.png')

%% Test 1
% simulation with different tForceStop
tForceStop = 1;
net = load("pinn_modela_128n.mat").netTrained;
...
function loss = modelLoss(Y,T) %, gradients] net,
    % make prediction
    %Y = forward(net,X);
    dataLoss = l2loss(Y,T);
